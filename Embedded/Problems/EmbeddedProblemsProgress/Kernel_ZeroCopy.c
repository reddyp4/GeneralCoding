/* Zero copy method is to prevent a user memory buffer and switching */
#include<unistd.h>
#include<stdio.h>
#include<fcntl.h>
...
int main(int argc, char** argv)
{
    if(argc!=4)
    {
        printf("usage:%s <source> <destintion> <mode>\n",argv[0]);
        exit(1);
    }
...
void zero_copy(const char *src_path, const char *dst_path)
{
    ...
    dst = open(dst_path, O_RDONLY|o_creat, stat_buf, st_mode);
