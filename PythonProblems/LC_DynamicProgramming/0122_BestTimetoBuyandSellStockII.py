...
class Solution:
    ...
    #function returns the max, based on the next biggest rise
    def recursive(self, prices,currGain):
        #print("prices:",prices,"currGain:",currGain)
        if(len(prices)==1 or len(prices)==0):
            return 0
        #get max of element 0 only and move on to next matrix
        maxGain=0
        for index,elem in enumerate(prices[:len(prices)-1]):
            #print("Loop: index:",index,"prices:",prices,"elem:",elem,"maxGain:",maxGain)
            #start comparing elem to rest of prices[0]
            gainIndex,newMin=0,elem
            # For each index, get the next max, and call recursive
            for k in range(index+1,len(prices)):
                if(prices[k]>newMin):
                    #gain found
                    gainIndex=prices[k]-newMin
                    #print("Found:",prices[k],"newMin:",newMin)
                    #Now call recursively for rest of prices for this
                    gainIndex+=self.recursive(prices[(k+1):],gainIndex)
                maxGain = maxGain if (maxGain>gainIndex) else gainIndex
                #print("Loop2: index:",index,"prices:",prices,"elem:",elem,"maxGain:",maxGain)
            #print("Loop3: index:",index,"prices:",prices,"elem:",elem,"maxGain:",maxGain)
        #print("Returning maxGain:",maxGain)
        return maxGain

    def maxProfit(self, prices: List[int]) -> int:
        #Recursive call for every element, if elem>stock, +=gain
        finalGain = self.recursive(prices,0)
        return finalGain
'''
...
class Solution:
    #function returns the max, based on the next biggest rise
    def recursive(self, prices,currIndex,currGain,dp):
        if(currIndex in dp):
            print("Found in dp!")
            return dp[currIndex]
        #print("prices1:",prices,"currIndex:",currIndex,"currGain:",currGain,"dp:",dp)
        #print("len(dp):",len(dp))
        if(len(prices)==0):
            #print("Reached 0, currIndex:",currIndex,"dp:",dp)
            dp[currIndex]=0
            return 0
        if(len(prices)==1):
            dp[currIndex]=0
            #print("End: prices:",prices,"dp:",dp)
            return 0
        #get max of element 0 only and move on to next matrix
        maxGain=0
        for index,elem in enumerate(prices[:len(prices)-1]):
            ...
            for k in range(index+1,len(prices)):
                if(prices[k]>newMin):
                    ...
                    gainIndex=prices[k]-newMin
                    #Now call recursively for rest of prices for this
                    if(currIndex+k+1) in dp:
                        print("Found in dp - index:value",k+1,":",dp[currIndex+k+1],"in ",prices,"dp:",dp)
                        gainIndex+=dp[currIndex+k+1]
                    else:
                        gainIndex+=self.recursive(prices[(k+1):],currIndex+k+1,gainIndex,dp)
                maxGain = maxGain if (maxGain>gainIndex) else gainIndex
                #print("Loop2: index:",index,"prices:",prices,"elem:",elem,"maxGain:",maxGain)
            #print("Loop3: index:",index,"prices:",prices,"elem:",elem,"maxGain:",maxGain)
        dp[currIndex]=maxGain
        #print("prices2:",prices,"dp:",dp,"currIndex",currIndex,"maxGain:",maxGain)
        return maxGain

    def maxProfit(self, prices: List[int]) -> int:
        #Recursive call for every element, if elem>stock, +=gain
        dp={}
        finalGain = self.recursive(prices,0,0,dp)
        return finalGain
'''
Recursion
prices1: [1, 2, 3, 4, 5] currIndex: 0 currGain: 0 dp: {5: 0, 6: 0, 3: 3, 4: 0, 0: 7}
len(dp): 5
prices1: [3, 4, 5] currIndex: 2 currGain: 1 dp: {5: 0, 6: 0, 3: 3, 4: 0, 0: 7}
len(dp): 5
prices2: [3, 4, 5] dp: {5: 0, 6: 0, 3: 3, 4: 0, 0: 7, 2: 2} currIndex 2 maxGain: 2
prices2: [1, 2, 3, 4, 5] dp: {5: 0, 6: 0, 3: 3, 4: 0, 0: 5, 2: 2} currIndex 0 maxGain: 5
'''